...
for(pf in unique(df$particle_factor))
  print(subset(df, particle_factor == pf) %>%
        plt(.) + ggtitle(paste("Particle factor", pf)))

# Coefficient of determination
calc_rsq <- function(pp1, pp2) summary(lm(pp2 ~ pp1 + 0))$r.squared

rsq <- group_by(df, type, proposal, n_taxa, particle_factor) %>%
...
asdsf <- transform(asdsf,
                   proposal = proposal_method_name,
                   n_trimmed = paste('trim', trim_count),
                   n_taxa = paste(n_taxa, 'taxa'),
                   type_factor = ifelse(type == 'mrbayes-mrbayes', type, paste(type, particle_factor)))

p <- ggplot(asdsf, aes(x = proposal, y = asdsf, fill=type_factor)) +
  geom_boxplot() +
  geom_hline(yintercept = 0.01, linetype = 'dashed') +
  facet_grid(n_taxa ~ n_trimmed, scales = 'free_y') +
  theme(legend.position = 'bottom')
print(p)
```

(note changing y scale)
